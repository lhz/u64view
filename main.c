...
int u64_command(int command, unsigned char *data, int len) {
    ...
    if (send(sock, &cmd[0], 4, 0) >= 0) {
        if (len == 0 || (len > 0 && send(sock, data, len, 0) >= 0)) {
            rv = 0;
        }
    }
...
// Send reset command to the U64
void u64_reset() {
    u64_do(SOCKET_CMD_RESET, "RESET");
}


// Send commands to enable VIC and SID streams to the U64
void u64_startstreams() {
    u64_do(SOCKET_CMD_VICSTREAM_ENABLE, "VICSTREAM_ENABLE");
    u64_do(SOCKET_CMD_SIDSTREAM_ENABLE, "SIDSTREAM_ENABLE");
}


// Send commands to disable VIC and SID streams to the U64
void u64_stopstreams() {
    u64_do(SOCKET_CMD_VICSTREAM_DISABLE, "VICSTREAM_DISABLE");
    u64_do(SOCKET_CMD_SIDSTREAM_DISABLE, "SIDSTREAM_DISABLE");
}


// Send keyboard command with the given PETSCII code to the U64
...
// Lookup PETSCII or action code in keyboard mapping table
